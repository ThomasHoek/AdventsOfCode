...
for i in task_input:
    ...
    while True:
        ...
        output = i + task_input[math.floor(num_lst)]

        if output == 2020:
            combined_numbers.append([i, task_input[math.floor(num_lst)]])
            break

        elif output > 2020:  # bigger then
            second_number = math.floor(np.mean(num_lst))
...
combined_numbers = [[lst[0], lst[1], lst[0]+lst[1], lst[0] * lst[1]]
