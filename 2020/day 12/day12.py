...
class boat():
    ...
    def change_direction(self, new_direction,amount):
        ...
        amount = (amount // 90) % 4
        if new_direction == "L":
            self.direction = direction_cycle[(direction_cycle.index(self.direction) - amount) % 4]    
    ...
    def output(self):
        return (self.xcoord, self.ycoord, self.direction, abs(self.xcoord) + abs(self.ycoord))
